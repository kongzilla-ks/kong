...
pub(crate) async fn icp_transfer(
    amount: &Nat,
    ...
    let transfer_args = TransferArgs {
        ...
        fee: DEFAULT_FEE,
        to: *to_account_id,
        created_at_time: created_at_time.map(|ts| Timestamp{timestamp_nanos: ts }),
    };

    match transfer(
        *token
            .canister_id()
            .ok_or(InternalTransferError::General("Invalid principal id".to_string()))?,
        &transfer_args,
    )
    .await
...
pub(crate) async fn icrc1_transfer(
    amount: &Nat,
