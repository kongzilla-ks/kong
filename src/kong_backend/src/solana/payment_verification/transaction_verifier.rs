...
pub async fn verify_solana_transaction(
    ...
    if let Some(metadata_json) = &transaction.metadata {
        ...
        let block_time = metadata
            .get("blockTime")
            .and_then(|v| v.as_u64())
            .ok_or_else(|| format!("Solana transaction {} missing transaction blockTime", tx_signature))?;
        ...
        return Err(format!("Solana transaction {} missing transaction metadata", tx_signature));
    }
